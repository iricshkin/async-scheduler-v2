...
class Scheduler:
    ...
    def run(self):
        while True:
            try:
                ...
                if job := self.queue.pop():
                    ...
                    if not gen:
                        ...
                        if not self.is_running:
                            # собираем задачи, которые еще не начали
                            # выполняться
                            self.not_completed_jobs.append(job)
                            continue

                        # если у задачи есть зависимости и не все они
                        # выполнены, не выполняем задачу
                        if (
                            job.dependencies
                            and not self._check_all_dep_complete(job)
                        ):
                            continue
                    ...
                    # если не упало StopIteration, значит задача не полностью
                    # выполнена, вставляем ее в конец очереди
                    self.add_job(job)
            except StopIteration:
                # задача полностью выполнена
                self.completed_job_list.append(name)
            except PrematureStartError:
                # если задача запущена раньше запланированного времени,
                # отправляем ее обратно в очередь
                self.add_job(job)
            except IndexError:
                ...
                logging.info(
                    f"job {name} is fail, attempts retries {job.retries}"
                )
                # в случае ошибки при выполнении задачи, смотрим на количество
                # ретраев
                if job.retries > 0:
                    # если еще остались попытки, снова добавляем задачу в
                    # очередь
                    self.add_job(job)
                    job.retries -= 1
    ...
    def stop(self):
        ...
        # чтобы уже запущенные задачи отработали, а не запущенные были собраны
        # в отдельный список
        self.run()
